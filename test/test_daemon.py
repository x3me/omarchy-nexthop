...
class TheDailySustainedPass(unittest.TestCase):
    """A line fast enough to outrun the hourly check's window is measured
    properly once a day — not hourly, which would be 55 GB a month and the
    request size Cloudflare refuses first."""
    ...
    def check(self, ago, detail="", network=None, down=400.0):
        self.d.store.put_test(int(self.now - ago), "content", "cloudflare",
                              down_mbps=down, up_mbps=50.0, ok=True,
                              detail=detail,
                              network=self.NET if network is None else network)

    def due(self, hint=400.0):
        return self.d._sustained_due(self.NET, hint, self.now)

    def test_a_slower_line_never_needs_one(self):
        # Below the rate at which the cap binds the hourly check already
        # holds its whole window; a longer pass would cost bytes for nothing.
        from nexthopd import speedtest
        self.assertLess(100.0, speedtest.cap_binds_above_mbps())
        self.assertFalse(self.due(hint=100.0))
        self.assertFalse(self.due(hint=None))

    def test_a_fast_line_with_no_history_gets_one(self):
        self.assertTrue(self.due())

    def test_not_twice_in_a_day(self):
        self.check(3600, detail="sustained")
        self.assertFalse(self.due())
        self.check(2 * 3600)                      # ordinary checks since
        self.assertFalse(self.due())
